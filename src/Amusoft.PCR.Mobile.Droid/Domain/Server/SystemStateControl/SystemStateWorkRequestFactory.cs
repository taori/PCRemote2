...
namespace Amusoft.PCR.Mobile.Droid.Domain.Server.SystemStateControl
{
	...
	{
		public static async Task EnqueueAsync<TWorker>(Context context, string address, SystemStateKind stateKind, DateTime date)
			where TWorker : DelayedSystemStateWorker
		{
			...
			WorkManager.GetInstance(context).EnqueueUniqueWork(stateKind + "+" + address, ExistingWorkPolicy.Replace, workRequest);
		}
	}
}